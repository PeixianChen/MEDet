...
class DeticFastRCNNOutputLayers(FastRCNNOutputLayers):
    ...
    def sigmoid_cross_entropy_loss(self, pred_class_logits, gt_classes):
        ...
        if self.ignore_zero_cats and (self.freq_weight is not None):
            self.freq_weight = torch.cat([self.freq_weight[self.freq_weight!=0], torch.zeros([C-48]).cuda()], dim=0)
            # self.freq_weight = self.freq_weight[self.freq_weight!=0]
            w = (self.freq_weight.view(-1) > 1e-4).float()
            # freq_weight = torch.cat([self.freq_weight[self.freq_weight!=0], torch.ones([C-48]).cuda()*10], dim=0)
    ...
    def forward(self, x, classifier_info=(None,None,None), inference=False, cls_weight=None):
        """
        ...
        if classifier_info[0] is not None:
            cls_scores, img_embds = self.cls_score(x, classifier=classifier_info[0], inference=inference)
            scores.append(cls_scores)
        else:
            cls_scores, img_embds = self.cls_score(x, inference=inference, cls_weight=cls_weight)
            scores.append(cls_scores)

        if classifier_info[2] is not None:
            cap_cls = classifier_info[2]
            if self.sync_caption_batch:
                caption_scores, _ = self.cls_score(x, classifier=cap_cls[:, :-1], inference=inference) 
            else:
                caption_scores, _ = self.cls_score(x, classifier=cap_cls, inference=inference)
        ...
        if self.with_softmax_prop:
            ...
            return scores, proposal_deltas, prop_score, img_embds, self.cls_score.zs_weight[:, :49]
        else:
            return scores, proposal_deltas, img_embds, self.cls_score.zs_weight[:, :49]
