...
from .SCAN import SCAN, cosine_similarity
from .contrastiveloss import ContrastiveLoss
from .match_concept_region import concept_region
...
class CustomRes5ROIHeads(Res5ROIHeads):
    ...
    def noise_removal(self, iteration, proposals, predictions, caption_words, caption_features, all_features, classifier_info, cap_file_name=None, TOPK=3):
        ...
        # indb = 0
        # # write the caption data bbox into json:
        # for indw, words in enumerate(caption_word_f):
        #     if words != []:
        #         name = os.path.basename(cap_file_name[indw])
        #         # self.caption_bbox[name] = words #, add_proposal_boxes[indb].tensor.detach().cpu().numpy()]
        #         self.caption_txt.write(name + " " + " ".join(words) + "\n")
        #         # self.bbox_numpy = np.concatenate([self.bbox_numpy, add_proposal_boxes[indb].tensor.detach().cpu().numpy()], axis=0)
        #         np.save("./bbox_npy/" + name[:-4] + ".npy", add_proposal_boxes[indb].tensor.detach().cpu().numpy())
        #         indb += 1
        #         if name not in all_img_name:
        #             # print("no name:", name)
        #             continue
        #         all_img_name.remove(name)
        
        # import json
        # with open('./caption_bbox_.json', 'w') as outfile:
            # json.dump(self.caption_bbox, outfile)
        # np.save("save_bbox.npy", self.bbox_numpy)
        # print(len(all_img_name), self.bbox_numpy.shape)
        # if len(all_img_name) < 55000 and len(all_img_name) > 50000:
        #     with open('./caption_bbox_55000.json', 'w') as outfile:
        #         json.dump(self.caption_bbox, outfile)
        # if len(all_img_name) < 45000 and len(all_img_name) > 40000:
        #     with open('./caption_bbox_45000.json', 'w') as outfile:
        #         json.dump(self.caption_bbox, outfile)  
        # if len(all_img_name) < 35000 and len(all_img_name) > 30000:
        #     with open('./caption_bbox_35000.json', 'w') as outfile:
        #         json.dump(self.caption_bbox, outfile)  
