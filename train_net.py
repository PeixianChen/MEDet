...
def do_train(cfg, model, resume=False):
    ...
    if not resume or (not os.path.exists(os.path.join(cfg.OUTPUT_DIR, "last_checkpoint"))):
        start_iter = 0
    ...
    with EventStorage(start_iter) as storage:
        ...
        for data, iteration in zip(data_loader, range(start_iter, max_iter)):
            ...
            if iteration > 0 and \
                ...
                periodic_checkpointer.save(str(iteration).zfill(8) + "_last_checkpoint", **cfg)
                # periodic_checkpointer.save("last_checkpoint", **cfg)
